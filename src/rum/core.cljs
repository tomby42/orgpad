(ns rum.core
  ...
  (:require
    ...
    [rum.util :as util :refer [next-id collect collect* call-all]]
...
(defn- build-ctor [render mixins display-name]
  (let [class  (build-class render mixins display-name)
        ...
        ctor   (if (some? key-fn)
                 (fn [& args]
                   (let [props #js { ":rum/initial-state" { :rum/args (vec args) }
...
(defn build-defc [render-body mixins display-name]
  (if (empty? mixins)
    (let [class (fn [props]
          ...
          ctor  (fn [& args]
                  (js/React.createElement class #js { ":rum/args" (vec args) }))]
      (with-meta ctor { :rum/class class }))
...
(defn- render-all [queue]
  (let [comp (reduce (fn [selected-comp comp] (if (< (id selected-comp) (id comp)) selected-comp comp)) queue)]
    ;; (js/window.console.log "Rendering React component " (id comp))
    (when (.isMounted comp)
      (.forceUpdate comp))))
